...
- Before maturity, accrued yield can be claimed incrementally by YT holders. `YTToken.claim_yield()` implements this today; it is not yet reachable end-to-end because `PrincipalManager` doesn't mint through `YTToken` yet (see §6.2).
...
### 5.5 Yield claiming (implemented in YTToken; not yet reachable end-to-end)
...
Every balance-changing operation (mint, burn, transfer in, transfer out) settles the affected account's pending yield at its balance *before* the change, against the current index, and only then advances that account's snapshot — otherwise a buyer could retroactively receive yield accrued before they held the position, or a seller could lose yield already earned by transferring out. `claim_yield()` settles the caller, then returns and zeroes their accumulated pending amount. In the current implementation this is computed and zeroed but not dispatched as a real underlying transfer — the same "computed, not dispatched" convention `PrincipalManager.redeem()` already uses, pending the `Router` integration that wires these contracts to actually move funds.

---

## 6. Standalone PT and YT Token Contracts

`PrincipalManager` tracks PT and YT balances internally in its own storage. `PTToken` and `YTToken` are implemented as standalone SEP-41 token contracts, correct and fully tested in isolation — but `PrincipalManager` does not yet call them; it still mints/burns against its own internal maps exactly as before these two contracts existed. Wiring `PrincipalManager` to actually mint/burn through `PTToken`/`YTToken` (so each instrument can be held in any Stellar wallet and traded on DEX pools) is outstanding, tracked alongside the `Router` integration.
