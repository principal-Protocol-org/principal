...
fn claim_yield(env, caller: Address, from: Address) -> i128;  // caller must be the registered minter
...
`claim_yield` settles `from` (see §1.3), then returns and zeroes their accumulated `PendingClaim`. `caller` must be the registered minter (`PrincipalManager`) — the same gate `mint`/`burn` already use. `claim_yield` used to authorize on `from` instead, making it a public entrypoint any holder could call directly; that settled and zeroed the claim but never transferred any underlying, a real economic footgun found during a follow-up audit. `PrincipalManager.claim_yield(from)` is now the only path that reaches it: it brings the index current, claims through this contract as the minter, and pays the result out via `SYWrapper.withdraw` in the same call, so a settled claim can never go unpaid. `PrincipalManager.redeem` also calls this directly as the authoritative payer of YT yield at redemption (rather than computing an independent amount itself), and since `claim_yield` is minter-gated there is no independent second payer to conflict with — see `contracts/principal_manager/src/lib.rs`'s module doc comment, and `redeem_yt_does_not_double_pay_yield_already_claimed_via_claim_yield` for the regression test.

`initialize` reads its genesis yield-index baseline (`LastOracleRate`) live from the oracle rather than hardcoding it to `SCALE`, and requires the oracle to be fresh at that moment (`OracleStale` otherwise) — found during a follow-up audit: a market created when the real rate is already above `SCALE` must not baseline against a value the rate never actually was.

Errors: same set as PTToken, plus `OracleStale` (`update_yield_index` or `initialize` called when the oracle isn't fresh).
...
`finalize_yt` used to need one thing `finalize_pt` didn't: `PrincipalManager.redeem` calls `YTToken.claim_yield(from=escrow)` two call frames below `finalize_yt` (`RecoveryEscrow -> PrincipalManager -> YTToken`), and `claim_yield` required that `from` address's own authorization. A contract's self-authorization only automatically covers calls it makes *directly*; reaching one frame further required explicitly pre-declaring that sub-invocation via `env.authorize_as_current_contract` before calling `redeem`. Now that `claim_yield` is minter-gated instead (§1.2) — authorized on `PrincipalManager`'s own address, the contract that actually calls it directly, one frame up — that workaround is no longer needed; `finalize_pt` and `finalize_yt` are symmetric, both relying on the same minter-gating `PTToken.burn`/`YTToken.burn` already used.

### 2.1 `PrincipalManager` now takes real custody -- `mint`/`redeem` call the real contracts

`mint` checks oracle freshness (`assert_oracle_fresh`, matching the check `redeem` already had) and calls `YTToken.update_yield_index()` before crediting the new YT balance -- found during a follow-up audit that `mint` previously read the oracle directly with no freshness check, and that skipping the index update meant a fresh mint's yield snapshot could baseline against a stale factor. It then transfers the caller's SY shares into `PrincipalManager`'s own custody via `SYWrapper.transfer` (a new SEP-41-style function added specifically for this -- SYWrapper previously had no share-transfer capability at all, only `deposit`/`withdraw`), then mints real `PTToken`/`YTToken` balances. `redeem` burns those real balances and releases real underlying via `SYWrapper.withdraw`, self-authorizing as `PrincipalManager`'s own contract address the same way `RecoveryEscrow` does when unwrapping a seizure (§3). `PrincipalManager`'s own address is therefore now a genuine SY holder between mint and redemption, and must itself be SAC-authorized and Permissioning-granted for that reason (see DEPLOYMENT.md).

`initialize` also cross-validates that `sy_wrapper`/`pt_token`/`yt_token` all share the configured `underlying`, `permissioning`, and (for PT/YT) `maturity`, and that `yt_token`'s oracle matches -- reverting `TopologyMismatch` otherwise. Without this, a deployment mistake could pair PT/YT from one market with SY custody, permissioning, or oracle assumptions from another; the check mirrors `RecoveryEscrow.initialize`'s existing underlying-consistency precedent (above), extended to permissioning, maturity, and oracle.

PT redemption keeps `PrincipalManager`'s own `pt_amount * SCALE / final_rate` formula (verified correct against the underlying economics; there is no independent PT-side payer to conflict with). YT redemption does not: it now calls `YTToken.update_yield_index()` then `burn()` then `claim_yield(caller=self, from)` and treats the returned amount as authoritative, instead of computing a second, independent payout the way an earlier version of this contract did. The reason is `claim_yield` is minter-gated (§1.2) backed by its own index -- `PrincipalManager` is the only caller that can ever reach it, so there is no independent second payer to reconcile against. `PrincipalManager.claim_yield(from)` provides the same settle-and-pay mechanism ahead of redemption, without burning YT or requiring maturity, and a later `redeem()` correctly pays nothing further for yield already claimed this way. See `contracts/principal_manager/src/lib.rs`'s module doc comment and the `redeem_yt_does_not_double_pay_yield_already_claimed_via_claim_yield` regression test.
